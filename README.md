...
本扩展支持使用酒馆默认预设（Default），对默认预设进行如下修改后即可使用：

- 修改`最大回复长度`为：65535
- 根据您使用的模型调整`上下文长度`为以下数值：
  - gemini：1000k
  - claude：800k
  - gpt：450k
  - grok：250k
...
本项目也兼容其他预设，但请注意根据使用的模型调整`上下文长度`
...
| `enableWordCountControl` | boolean | true | 启用字数控制 |
...
| 名称 | 正则表达式 | 用途 |
|------|-----------|------|
| hideThoughtChain | `<think[\s\S]*?</think\|<thinking>[\s\S]*?</thinking\|\*\*Thinking about your request\*\*` | 隐藏思维链内容 |
| hideDisclaimer | `<disclaimer[\s\S]*?</disclaimer\|<Statement>[\s\S]*?</Statement\|<Word Count Requirement>[\s\S]*?</Word Count Requirement\|<safe>[\s\S]*?</safe\|<null>[\s\S]*?</null>` | 隐藏免责声明和扩展标签 |
...
async function loadTemplateConfig() {
    ...
    for (const [key, value] of Object.entries(templateConfig.regexPatterns)) {
        try {
            ...
            console.error(`Failed to compile regex pattern '${key}':`, error);
        }
    }
...
```mermaid
graph TB
    A[扩展加载] --> B[读取本地manifest.json]
    B --> C[获取本地版本号]
    C --> D[请求GitHub远程manifest.json]
    D --> E{版本比较}
    E -->|远程版本更高| F[显示NEW标签]
    E -->|版本相同或更低| G[不显示提示]
    F --> H[标题栏NEW标签]
...
**版本号管理**：
- 版本号唯一来源：`manifest.json` 中的 `version` 字段
- 本地版本通过 `getLocalVersion()` 从本地 manifest.json 动态获取
- 远程版本通过 `getLatestVersion()` 从 GitHub 获取
- 修改版本号只需修改 `manifest.json` 一处，降低维护成本
